...
> **Note (2026-09-15):** Added an **identity layer** (`src/Identity/`) — session hijack, unusual login, data tamper, and login brute-force lockout detection. These need cross-request state, so they run *outside* `DetectorChain` and merge into the same threat list via `SecurityGuard::guard()`. Cookie sessions and token logins (`Authorization: Bearer` / `X-Token`) share one code path; raw session IDs and tokens are persisted and logged as `sha256` hashes only. Opt-in via the `identity` config block. Two further additions landed alongside it: `NormalizationScanner` re-scans decoded payload variants (URL/double-encoding, fullwidth, HTML entities), and `SecurityGuard::securityHeaders()` injects hardening response headers on both normal and blocked responses. Two later fixes landed on the same day: all five request-source extraction sites go through `SecurityGuard::mergeRequestSources()`, so a name carried by two sources keeps **both** values (the displaced one scanned under `_<source>.<name>`) instead of `array_merge()` silently dropping one; and `UploadDetector` was restored — its `$_FILES`-shaped match never fired inside `guard()`, because `flattenData()` had already split the entry into `field.name` / `field.tmp_name`. The suite is now **427 tests, 1041 assertions**. The File Map below is the v1 plan's map and is left as-is.

> **Follow-up (2026-09-15, v1.3.1):** the framework-free path in `src/helpers.php` was brought to parity with the four middlewares. `security_guard()` now emits `security_headers` *before* scanning, so both the blocked and the passing response carry them, and dies with the threat's own status code rather than always 403. `security_scan_current_request()` now passes `cookies`, `user_agent` and the `identity.session.headers` token headers inside `$meta` — the exact keys `SessionFingerprint` reads — so session-hijack and token-login detection work outside a framework too; before this they were silently dead there (`extractSessionId()` found nothing and returned `null` on every request). `Authorization` falls back to `getallheaders()` under Apache+CGI, where it never reaches `$_SERVER`. The Step 1 `security_guard()` snippet below is the frozen v1 text; see the spec for the current contract. Suite: **406 tests, 996 assertions**.

> **Follow-up (2026-09-25, v1.4.0 + v1.4.1):** two releases on one day. **v1.4.0** — the block response now varies with the client: `SecurityGuard::blockResponse()` renders `src/BlockPage.php` (mascot 小盾 + status + the detector names that fired) when `Accept` contains `text/html`, and keeps the plain-text `block_message` for API/curl/fetch clients; `block_page.enabled` (missing key = on) disables the page, and all four middlewares plus `security_guard()` share the one method. The same release added the Composer-free install path — `prepend.php` for `auto_prepend_file`, a PSR-4 fallback loader inside `src/helpers.php` so a bare `require` works, and `SECURITY_CONFIG` to point at your own config copy — plus a prefilter gate in `AbstractRegexDetector` (patterns grouped by flag set, one combined alternation per group, applied below 8KB; only a definite `0` skips a group, `false` still runs every pattern). **v1.4.1** — hardening of the stateful side: `FileStorage` reads under `LOCK_SH` (a write truncates in place, so an unlocked reader could see an empty file and read it as "no state") and reports a failed persist once per path per process via `error_log`; `IpBlacklist` reclaims entries whose window expired below the threshold and a new window no longer resets an active ban; `SessionFingerprint` refreshes its idle timer at most every `ttl/2` instead of rewriting the whole store per request; `IpPrefix` masks IPv4-mapped IPv6 (`::ffff:v4`) as IPv4 so mapped clients stop collapsing into one `::/64`; `Logger` appends a random suffix when two rotations land in the same second; `NormalizationScanner` decodes named entities too (`&lt;script&gt;` reached no detector — the gate only looked for `&#`); `RedisStorage::all()` issues one `MGET` per SCAN batch. The File Map below is the v1 plan's and stays frozen. Suite: **451 tests, 31603 assertions**.

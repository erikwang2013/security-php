...
return [
     ...
     * 计数窗口过期且未达阈值的记录会被删除，存储不随攻击过的 IP 数增长；
     * 仍在生效的封禁不会被新窗口重置。
     ...
     *
     * 写入失败（目录不可写、磁盘满、编码失败）会用 error_log 报一次：
     * 每个路径在每个进程里只报一次，不会刷屏，但静默失败意味着
     * 封禁、锁定与会话绑定全部失灵，排查时先看这条日志。
     */
    'storage' => [
        'type' => 'file',

        // File 存储配置（type=file 时生效）
        // 读写都不裸奔：写入加排他锁并原地截断，读取加共享锁 —— 否则读到截断中的
        // 空文件会被当成「还没有状态」（新会话基线、清零的失败计数、不存在的封禁）。
